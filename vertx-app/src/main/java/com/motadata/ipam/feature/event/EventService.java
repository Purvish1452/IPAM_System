package com.motadata.ipam.feature.event;
        ...
        return db.query(countSql).execute().compose(countRows -> {
            ...
            return db.preparedQuery(dataSql).execute(Tuple.of(size, offset)).map(dataRows -> {
                ...
                for (Row row : dataRows) {
                    ...
                    long epochMillis = ts != null ? ts.atZone(ZoneId.systemDefault()).toInstant().toEpochMilli() : System.currentTimeMillis();
