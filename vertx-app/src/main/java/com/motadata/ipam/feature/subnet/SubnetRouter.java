...
public class SubnetRouter {
    ...
    private void handleImportSubnetIPCSV(RoutingContext ctx) {
        ...
        String uploadedFile = uploads.get(0).uploadedFileName();
        ctx.vertx().fileSystem().readFile(uploadedFile)
                .onSuccess(buffer -> {
                    byte[] csvBytes = buffer.getBytes();
                    if (csvBytes.length == 0) {
                        ctx.response().putHeader("Content-Type", "application/json;charset=UTF-8")
                                .end(new JsonObject().put("success", false).put("message", "Uploaded CSV file is empty").encode());
                        return;
                    }

                    LOGGER.info("Import CSV for subnetId={}, size={} bytes", sid, csvBytes.length);
                    ipActionService.importIPsFromCSV(csvBytes, sid).onComplete(ar -> {
                        JsonObject result = ar.succeeded() ? ar.result() :
                                new JsonObject().put("success", false).put("message", "CSV import failed: " + (ar.cause() != null ? ar.cause().getMessage() : "Unknown error"));
                        ctx.response().putHeader("Content-Type", "application/json;charset=UTF-8").end(result.encode());
                    });
                })
                .onFailure(err -> {
                    LOGGER.error("Failed to read uploaded CSV file: {}", err.getMessage());
                    ctx.response().putHeader("Content-Type", "application/json;charset=UTF-8")
                            .end(new JsonObject().put("success", false).put("message", "Failed to read uploaded CSV file").encode());
                });
    }
