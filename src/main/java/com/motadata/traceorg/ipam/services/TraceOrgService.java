package com.motadata.traceorg.ipam.services;
...
public interface TraceOrgService {
	...
	List<?> commonQuery(String prefix, int pageSize, int offset);

	List<?> commonQuery(String prefix);
	
	List<?> sqlQuery(String query);

	boolean sqlQueryAction(String query);

	void switchSafeUpdateMode(int onOffToggle);
